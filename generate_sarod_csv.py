...
for freq in notes:
    for amp in amplitudes:
        sg, g = interpolate(freq, amp, orig_data)
        new_rows.append({'frequency': freq, 'amplitude': amp, 'symp_gain': sg, 'gain': g})

# Write back
with open('assets/dsp/sarod.csv', 'w', newline='') as f:
    writer = csv.DictWriter(f, fieldnames=['frequency', 'amplitude', 'symp_gain', 'gain'])
    writer.writeheader()
    for row in new_rows:
        writer.writerow(row)
