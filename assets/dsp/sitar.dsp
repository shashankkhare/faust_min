...
// FIXED LOGIC: Chikari instances are now driven directly by the master excitation channel.
// Setting 'strike = 1' and clicking 'gate' will now cleanly fire the drone overtones reliably every single time.
chikariLoop = ( sitar_string(chikari_freq1, 1.8, jivari * 0.7, 0.35, is_chikari, master_exc_signal)
              + sitar_string(chikari_freq2, 1.8, jivari * 0.7, 0.35, is_chikari, master_exc_signal)
              ) * 0.5;
...
body_filter(x) =
    (  (x * 0.25) 
     ...
     + (x : fi.resonbp(2200.0, 3.5, 0.7)) 
     + (x : fi.resonbp(3600.0, 3.0, 0.4)) 
    );
