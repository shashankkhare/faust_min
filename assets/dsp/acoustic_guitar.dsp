...
// (HPcutoff, LPcutoff, sharpness, gain, trigger)
strikeTrigger = pm.strikeModel(100, 4000, 0.3, 1.0, gate);
...
steelModel = pm.guitar(stringLength, pluckPosition, velocity * excBoost*0.1, strikeTrigger)
             : fi.highshelf(1, 3.0, 3000.0);

nylonModel = pm.nylonGuitar(stringLength, pluckPosition, velocity * excBoost*0.1, strikeTrigger)
             : fi.lowpass(1, 2200.0);

bassModel  = pm.guitar(stringLength, pluckPosition, velocity * excBoost*0.1, strikeTrigger)
             : fi.lowpass(2, 750.0);
