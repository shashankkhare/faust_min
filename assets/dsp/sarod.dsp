...
chikari_gain = hslider("chikari_gain", 0.2, 0, 1, 0.01);
...
// Pluck envelope: slow linear up-ramp over 'pluck_atk' seconds to 1.0, then
...
with {
    dur = int((atk + rel) * ma.SR);
    count = loop ~ _
    with { loop(n) = ba.if(t, 0, min(n + 1, dur)); };
    tri(n) = ba.if(n <= int(atk * ma.SR),
                   n / max(1, int(atk * ma.SR)),
                   max(0.0, (dur - n) / max(1, int(rel * ma.SR))));
};
...
chikari_pluck_env = triangle_env(pluck_atk, pluck_rel, chikari_trig);
...
// CALIBRATED DECAY: Reduced max sustain from 4.0s down to 2.0s for tighter decay
dynSustain = 2.0 - normFreq * 0.85; 
...
body_filter(x) =
    (  (x * 0.25)                               // passthrough — carries membrane high-freqs
     + (x : fi.resonbp(180.0,  2.5,  1.60))    // low body warmth
     + (x : fi.resonbp(320.0,  1.5,  0.80))    // mid body resonance
     + (x : fi.resonbp(550.0,  2.0,  0.60))
     + (x : fi.resonbp(950.0,  3.5,  0.80))
     + (x : fi.resonbp(1050.0, 3.5,  0.85))
     + (x : fi.resonbp(1200.0, 3.0,  0.70))
     + (x : fi.resonbp(1800.0, 3.0,  0.45))
    );
...
process = summed : membrane_filter : body_filter : *(gain * 2.4995 * 5.14675) : ma.tanh <: _,_;
