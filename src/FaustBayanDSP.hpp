...
class FaustBayanDSP : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("strike", &fHslider1, FAUSTFLOAT(0.0f), FAUSTFLOAT(0.0f), FAUSTFLOAT(3.0f), FAUSTFLOAT(1.0f));
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		int iSlow3 = (iSlow1 >= 1);
		int iSlow4 = (iSlow1 >= 3);
		float fThen2 = (iSlow3 ? 1.5f : 1.20000005f);
		float fElse2 = (iSlow4 ? 1.5f : 1.0f);
		float fSlow5 = (iSlow2 ? fElse2 : fThen2);
		float fThen5 = (iSlow3 ? 0.200000003f : 0.5f);
		float fElse5 = (iSlow4 ? 0.100000001f : 0.400000006f);
		float fSlow6 = (iSlow2 ? fElse5 : fThen5);
		float fSlow7 = float(fHslider2);
		int iSlow8 = (iSlow1 == 0);
		int iSlow9 = (iSlow8 | (iSlow1 == 2));
		float fSlow10 = (1.0f / std::tan((fConst1 * ((1500.0f * fSlow7) + (iSlow9 ? 2000.0f : 800.0f)))));
		float fSlow11 = (1.0f - fSlow10);
		float fSlow12 = (fSlow10 + 1.0f);
		float fSlow13 = (0.0399999991f * fSlow7);
		float fThen7 = (fSlow13 + 0.930000007f);
		float fThen8 = (fSlow13 + 0.879999995f);
		float fThen9 = (iSlow3 ? 0.829999983f : fThen7);
		float fElse9 = (iSlow4 ? 0.829999983f : fThen8);
		float fSlow14 = (iSlow2 ? fElse9 : fThen9);
		float fSlow15 = (4.65661287e-10f * fSlow7);
		float fSlow16 = float(fButton0);
		float fSlow17 = std::max<float>(1.0f, (fConst0 * (iSlow9 ? 0.00200000009f : 0.00100000005f)));
		float fSlow18 = std::max<float>(1.0f, (fConst0 * (iSlow9 ? 0.0199999996f : 0.00999999978f)));
		float fSlow19 = (fConst24 * float(fHslider3));
		float fSlow20 = (fSlow7 * (std::pow(2.0f, (0.0833333358f * float(fHslider4))) + -1.0f));
		float fSlow21 = (iSlow8 ? 1.0f : 0.0f);
		float fSlow22 = (fConst0 / float(fHslider5));
		float fThen16 = (iSlow3 ? 0.800000012f : 0.5f);
		float fElse16 = (iSlow4 ? 0.899999976f : 0.600000024f);
		float fSlow23 = (iSlow2 ? fElse16 : fThen16);
		float fSlow24 = (0.25f * fSlow7);
		float fSlow25 = float(fHslider6);
		float fSlow26 = std::tan((fConst38 * std::sqrt(std::max<float>(9.99999975e-05f, (3.14159274f * (std::pow(fSlow25, 2.0f) / std::max<float>(9.99999975e-05f, (1.5f * (fSlow25 * float(fHslider7))))))))));
		float fSlow27 = (1.0f / fSlow26);
		float fSlow28 = (((fSlow27 + 0.5f) / fSlow26) + 1.0f);
		float fSlow29 = (1.5f / (fSlow26 * fSlow28));
		float fSlow30 = (1.0f / fSlow28);
		float fSlow31 = (((fSlow27 + -0.5f) / fSlow26) + 1.0f);
		float fSlow32 = (2.0f * (1.0f - (1.0f / FaustBayanDSP_faustpower2_f(fSlow26))));
		float fSlow33 = (0.0f - fSlow29);
		for (int i0 = 0; (i0 < count); i0 = (i0 + 1)) {
			float fTempFTZ0 = (0.0f - (((fRec1[1] * fSlow11) - (fRec0[1] + fRec0[2])) / fSlow12));
			fRec1[0] = ((std::fabs(fTempFTZ0) > 1.17549435e-38f) ? fTempFTZ0 : 0.0f);
			iRec4[0] = ((1103515245 * iRec4[1]) + 12345);
			fVec0[0] = fSlow16;
			iRec5[0] = ((fSlow16 > fVec0[1]) + ((fSlow16 <= fVec0[1]) * (iRec5[1] + (iRec5[1] > 0))));
			float fTemp0 = float(iRec5[0]);
			float fTemp1 = (fSlow15 * (float(iRec4[0]) * std::max<float>(0.0f, std::min<float>((fTemp0 / fSlow17), (1.0f - ((fTemp0 - fSlow17) / fSlow18))))));
			...
			float fThen12 = (fConst5 * (fRec2[2] + (fRec2[0] + (2.0f * fRec2[1]))));
			float fElse12 = (fConst20 * (fRec6[2] + (fRec6[0] + (2.0f * fRec6[1]))));
			float fTemp2 = (iSlow9 ? fElse12 : fThen12);
			float fTemp3 = ((fRec1[0] * fSlow14) + fTemp2);
			fVec1[(IOTA & 8191)] = fTemp3;
			float fTempFTZ4 = (fSlow19 + (fConst25 * fRec7[1]));
			fRec7[0] = ((std::fabs(fTempFTZ4) > 1.17549435e-38f) ? fTempFTZ4 : 0.0f);
			float fTemp4 = (fSlow21 * std::max<float>(0.0f, std::min<float>((fConst27 * fTemp0), (1.0f - (fConst28 * (fTemp0 - fConst26))))));
			float fTemp5 = (fRec7[0] * ((fSlow20 * fTemp4) + 1.0f));
			...
			float fTempFTZ6 = (0.0f - (((fSlow11 * fRec9[1]) - (fRec8[1] + fRec8[2])) / fSlow12));
			fRec9[0] = ((std::fabs(fTempFTZ6) > 1.17549435e-38f) ? fTempFTZ6 : 0.0f);
			float fTemp9 = (fTemp2 + (fRec9[0] * fSlow14));
			fVec2[(IOTA & 8191)] = fTemp9;
			float fTemp10 = (fSlow22 / fTemp5);
			...
			float fTemp13 = (fSlow24 + ((0.00100000005f * (200.0f - fTemp5)) + 0.150000006f));
			float fThen18 = (0.699999988f * fTemp13);
			float fThen19 = (iSlow3 ? 0.0299999993f : fTemp13);
			float fElse19 = (iSlow4 ? 0.0299999993f : fThen18);
			float fTemp14 = ((iSlow2 ? fElse19 : fThen19) * (1.0f - (0.850000024f * fTemp4)));
			...
			float fTemp20 = ((0.5f * (fSlow6 * (fRec0[0] + fRec8[0]))) + (fSlow23 * ((((0.800000012f * fRec10[0]) + (0.5f * fRec11[0])) + (0.300000012f * fRec12[0])) + (0.200000003f * fRec13[0]))));
			float fTempFTZ12 = (fTemp20 - (fSlow30 * ((fSlow31 * fRec15[2]) + (fSlow32 * fRec15[1]))));
			fRec15[0] = ((std::fabs(fTempFTZ12) > 1.17549435e-38f) ? fTempFTZ12 : 0.0f);
			float fTempFTZ13 = (((fSlow29 * fRec15[0]) + (fSlow33 * fRec15[2])) - (fConst39 * ((fConst40 * fRec14[2]) + (fConst41 * fRec14[1]))));
			fRec14[0] = ((std::fabs(fTempFTZ13) > 1.17549435e-38f) ? fTempFTZ13 : 0.0f);
			output0[i0] = FAUSTFLOAT((fSlow0 * float(tanhf(float((4.0f * (fSlow5 * (fTemp20 + (fConst37 * (fRec14[2] + (fRec14[0] + (2.0f * fRec14[1]))))))))))));
			fRec1[1] = fRec1[0];
			iRec4[1] = iRec4[0];
			fVec0[1] = fVec0[0];
			iRec5[1] = iRec5[0];
